...
class _ProfileCardState extends State<ProfileCard> {
  ...
  Widget build(BuildContext context) {
    ...
    _logger.debug('Building card for profile ${widget.profile.id}, selected photo index: $_selectedPhotoIndex');
                ...
                child: currentImageUrl != null
                  ? CachedNetworkImage(
                      ...
                      errorWidget: (context, url, error) {
                         _logger.error('Error loading main image: $url, Error: $error');
                         return Image.asset(
                           'assets/images/default_profile.png',
                            fit: BoxFit.cover,
                            errorBuilder: (context, error, stackTrace) {
                              _logger.error('Error loading default_profile.png: $error');
                  ...
                  : Image.asset(
                      ...
                      fit: BoxFit.cover,
                      errorBuilder: (context, error, stackTrace) {
                         _logger.error('Error loading default_profile.png (no photos): $error');
                         return const Center(child: Icon(Icons.person_off, size: 50, color: Colors.grey));
                       },
                  ),
              ),
            ),
                 ...
                 child: Row(
                   ...
                   children: photoUrls.asMap().entries.map((entry) {
                     ...
                     if (index >= photoUrls.length) {
                       _logger.warn('Thumbnail index $index out of bounds for photoUrls length ${photoUrls.length}');
                     ...
                     return GestureDetector(
                       onTap: () {
                         if (_selectedPhotoIndex != index) {
                            _logger.debug('Thumbnail tapped, changing index to $index');
                            setState(() {
                       ...
                       child: Container(
                         ...
                         child: ClipRRect(
                           ...
                           child: CachedNetworkImage(
                             ...
                             errorWidget: (context, url, error) {
                               _logger.error('Error loading thumbnail image: $url, Error: $error');
                               return Container(
                                 width: 50, height: 70,
                                 color: Colors.grey[300],
              ...
              children: [
                ...
                if (widget.profile.interests.isNotEmpty)
                  Wrap(
                    spacing: 6.0,
                    runSpacing: 4.0,
                    children: widget.profile.interests.map((interest) => Chip(
                      label: Text(interest),
