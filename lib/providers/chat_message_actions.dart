...
class ChatMessageActions {
  ...
  static void addMessage(WidgetRef ref, String conversationId, String text, {String? mediaUrl}) {
    ...
    if (!isConnected) {
      print("⚠️ WARNING: WebSocket not connected! Attempting to connect and send...");
      _attemptReconnectAndSend(ref, chatService, conversationId, text, mediaUrl: mediaUrl);
      return;
    }
  ...
  static void _attemptReconnectAndSend(WidgetRef ref, ChatService chatService, String conversationId, String text, {String? mediaUrl}) {
    ...
    chatService.initSocket().then((_) {
      ...
      Future.delayed(Duration(seconds: 2), () {
        final isConnected = chatService.isConnected;
        print("📡 WEBSOCKET RECONNECT STATUS: ${isConnected ? 'Connected' : 'Still Disconnected'}");
  ...
  static void toggleReaction(WidgetRef ref, String conversationId, String messageId, String emoji) {
    print("👍 REACTION: Toggling reaction $emoji on message $messageId in conversation $conversationId");
  ...
  static void sendTypingIndicator(WidgetRef ref, String conversationId) {
    print("⌨️ TYPING: Sending typing indicator for conversation $conversationId");
  ...
  static void stopTypingIndicator(WidgetRef ref, String conversationId) {
    print("⌨️ STOPPED TYPING: Stopping typing indicator for conversation $conversationId");
  ...
  static Future<void> initializeChat(WidgetRef ref) async {
    ...
    try {
      ...
      Future.delayed(Duration(seconds: 2), () {
        final isConnected = chatService.isConnected;
        print("📡 WEBSOCKET INIT STATUS: ${isConnected ? 'Connected' : 'Failed to Connect'}");
      });
    } catch (e) {
